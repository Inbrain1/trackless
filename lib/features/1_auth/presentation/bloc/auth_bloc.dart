...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc({required AuthRepository authRepository})
    ...
    _authStateSubscription = _authRepository.authStateChanges.listen((firebaseUser) {
  ...
  Future<void> _onAuthStateChanged(AuthStateChanged event, Emitter<AuthState> emit) async {
    ...
    if (firebaseUser != null) {
      ...
      final userDetails = await _authRepository.getUserDetails(firebaseUser.uid);
      if (userDetails != null) {
        // Si encontramos los detalles, emitimos un estado autenticado CON el usuario completo.
        emit(state.copyWith(status: AuthStatus.authenticated, user: userDetails));
      } else {
        // Si no se encuentran detalles (caso raro), lo marcamos como no autenticado.
        emit(state.copyWith(status: AuthStatus.unauthenticated, user: null, message: 'No se encontraron detalles del usuario.'));
      }
    } else {
  ...
  Future<void> _onSignInRequested(SignInRequested event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      if (user == null) {
        emit(state.copyWith(status: AuthStatus.unauthenticated, message: 'Credenciales incorrectas.'));
      }
      // No emitimos 'authenticated' aquí, el listener `authStateChanges` lo hará automáticamente
      // y de forma centralizada al detectar el cambio en Firebase.
    } catch (e) {
